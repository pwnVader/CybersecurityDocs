...
description: "WordPress, Joomla, Drupal, Tomcat, Jenkins y Splunk."
sidebar:
  order: 10
  label: "Attacking Common Applications"
---
> Enumeración y explotación de aplicaciones comunes: CMS (WordPress, Joomla, Drupal), servidores de aplicaciones (Tomcat, Jenkins), SIEM/monitorización (Splunk, PRTG), y herramientas colaborativas (GitLab, osTicket).
...
- **WPScan sin API key:** enumera plugins pero sin datos de vulnerabilidades. Registrar en wpvulndb.com para token gratuito (25 req/día).
- **WordPress theme inactive:** editar siempre un tema INACTIVO (ej. Twenty Nineteen) para no romper el sitio.
- **Joomla brute force:** la página de error es genérica ("Username and password do not match") → no revela si el usuario existe.
- **Drupal PHP Filter en v8+:** no instalado por defecto. Instalarlo cambia el sistema → pedir permiso al cliente.
- **Tomcat WAR undeploy:** siempre hacer `Undeploy` tras el pentest para limpiar la webshell.
- **Ghostcat (AJP):** solo lee archivos dentro de `webapps/`, no puede leer `/etc/passwd`.
- **Jenkins como SYSTEM/root:** muy común internamente. Script console → inmediato RCE como SYSTEM.
- **Splunk free/trial:** tras 60 días se convierte en Free sin autenticación. Buscar en redes internas.
- **PRTG — injection ciega:** el comando se ejecuta pero no hay output visible. Añadir usuario y verificar con crackmapexec.
- **GitLab user enumeration:** registro con username ya tomado → "is already taken". Válido incluso sin self-registration activo (browsing a /users/sign_up).
- **osTicket email abuse:** si el helpdesk asigna emails internos a tickets, se pueden usar para registrarse en servicios internos.
- **Artefactos post-pentest:** siempre documentar: webshells subidas, usuarios creados, archivos modificados. Listarlos en el apéndice del informe.

---

## Cheatsheets relacionados

- [File Upload Attacks](/metodologias/web/file-upload-attacks/) — WAR/zip/plugin upload → RCE en Tomcat, WordPress, Drupal
- [Login Brute Forcing](/metodologias/web/login-brute-forcing/) — brute force de paneles admin con hydra/wpscan/MSF
- [Using the Metasploit Framework](/metodologias/exploitation/metasploit-framework/) — módulos para Tomcat, Jenkins, WordPress, Drupal
- [SQL Injection Fundamentals](/metodologias/web/sql-injection-fundamentals/) — Drupalgeddon vía SQLi preauth
- [File Inclusion](/metodologias/web/file-inclusion/) — Ghostcat AJP para leer WEB-INF/web.xml
- [Active Directory Enumeration & Attacks](/metodologias/active-directory/active-directory-enumeration-attacks/) — Jenkins/Splunk como SYSTEM → foothold en AD
